...
From VyZX Require Import PermutationInstances.
Require Export MatEquivSetoid.
Require Export PermMatrixFacts.
...
Proof.
  ...
  unfold rotr; simplify_bools_lia; solve_simple_mod_eqns.
Qed.
...
Proof.
  ...
  restore_dims.
  rewrite kron_comm_commutes_vectors_l by auto with wf_db.
  rewrite perm_mat_permutes_ei_r by show_moddy_lt.
...
Proof.
  intros p q.
  rewrite (kron_comm_eq_perm_mat_of_kron_comm_perm p q).
  rewrite (kron_comm_eq_perm_mat_of_kron_comm_perm q p).
  rewrite Nat.mul_comm.
  rewrite perm_mat_Mmult by auto with perm_db.
  cleanup_perm_inv.
  now rewrite perm_mat_idn.
Qed.
...
Tactic Notation "restore_dims" "in" ident(H) :=
  match type of H with 
  | ?A => let A' := restore_dims_rec A in 
    replace A with A' in H by unify_matrix_dims
    ltac:((repeat rewrite Nat.pow_1_l); try ring; unify_pows_two; simpl; lia)
  end.
...
Proof.
  ...
  2: {
    ...
    2: {
      ...
      replace (ij / n <? m)%nat with true
        by (symmetry; rewrite Nat.ltb_lt; apply Nat.Div0.div_lt_upper_bound; lia).
  ...
  - rewrite Msum_Csum; apply big_sum_unique.
    ...
    + rewrite Nat.div_add_l, Nat.div_small, Nat.add_0_r, Nat.eqb_refl by lia.
      ...
      1: rewrite Nat.add_comm, Nat.Div0.mod_add, Nat.mod_small, Nat.eqb_refl by lia.
      all: bdestructΩ'simp.
...
Proof.
  ...
  2:{
    ...
    (try apply Nat.Div0.div_lt_upper_bound; try apply Nat.mod_upper_bound; lia).
...
Proof.
  ...
  rewrite 2!transpose_involutive.
  apply (kron_comm_mx_to_vec_mat_equiv q p (A⊤)).
Qed.
...
Proof.
  intros n m.
  unfold Nat.lcm.
  rewrite <- 2!Nat.Lcm0.divide_div_mul_exact, (Nat.mul_comm (Nat.gcd _ _)),
...
Proof.
  ...
  exists m.
  rewrite <- Nat.Lcm0.divide_div_mul_exact, Nat.mul_comm, Nat.div_mul; easy.
Qed.
...
Proof.
  ...
  assert (H: m <> O) by (intros Hfalse; subst m; rewrite div_0_r in *; lia).
  rewrite <- Nat.Lcm0.divide_div_mul_exact, Nat.mul_comm, Nat.div_mul
    by (try apply div_divides; easy). 
  now rewrite <- Nat.Lcm0.divide_div_mul_exact, Nat.mul_comm, Nat.div_mul.
Qed.