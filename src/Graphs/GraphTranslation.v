...
Lemma perm_of_list_bounded (l : list nat) (H : is_perm_list l = true) : 
...
Lemma invperm_of_list_bounded (l : list nat) (H : is_perm_list l = true) : 
...
Proof.
  ...
  intros k Hk.
  repeat split.
  - now apply perm_of_list_bounded.
  - now apply invperm_of_list_bounded.
...
Definition zx_of_list (l : list nat) :=
  ZXpermFacts.zx_of_perm (length l) (perm_of_list l).
...
Definition invzx_of_list (l : list nat) :=
  ZXpermFacts.zx_of_perm (length l) (invperm_of_list l).
...
Definition zx_between_lists (l l' : list nat) : ZX _ _ :=
  ZXpermFacts.zx_of_perm (length l) 
...
Proof. apply zx_of_perm_zxperm. Qed.
...
Module GraphToDiagram_biperm.

Export GraphToDiagram_prelim.


(* Reduce to the case where all the inputs and outputs to / from spiders; 
   should be simple pre-processing *)
...
(* Alternate program : generalize by making each spider have
   a list of all the spiders that spider outputs *to* (so an
   edge [a, b] is counted for a iff a < b). Optionally, pick
   to preprocess to remove self-loops; it's likely a special
   case if not. Either way, this seems like the right choice
   of general form of problem to solve, and we can then make
   similarly general statements with bipermutation machinery. *)

Import Modulus.

Section Multigraph_prelim.

(* For i < big_sum g n, returns the (necessarily unique) 
   k <= n such that big_sum g k <= i < big_sum g (S k) *)
Fixpoint big_sum_index (g : nat -> nat) (n : nat) (i : nat) : nat :=
  match n with 
  | 0 => 0
...
Lemma big_sum_index_spec g n i (Hi : i < big_sum g n) : 
  big_sum g (big_sum_index g n i) <= i < big_sum g (S (big_sum_index g n i)).
Proof.
  induction n; [cbn in Hi; lia|].
  cbn.
  bdestruct_one.
  - cbn in Hi; lia.
  - apply IHn; easy.
...
End Multigraph_prelim.

Section Simple_Multigraph.

Context (f : nat -> nat -> nat) (n : nat).
Context (Hf : forall i j, i < n -> j < n -> f i j = f j i).
Context (Hfdiag : forall i, i < n -> f i i = 0).

Definition degree i :=
  big_sum (f i) n.

Lemma degree_alt i : i < n ->
  degree i = big_sum (fun j => f j i) n.
Proof.
  intros Hi.
  unfold degree. 
...
Definition outsize i :=
  big_sum (shift (f i) (S i)) (n - S i).

Lemma degree_decomp i (Hi : i < n) : 
  degree i = insize i + outsize i.
Proof.
  unfold degree, insize, outsize.
  simpl.
  rewrite (big_sum_split n i), Hfdiag by easy.
  cbn.
  unfold shift.
  f_equal; [lia|].
  f_equal.
...
Lemma degree_alt i : i < n ->
  degree i = f i i + big_sum (fun j => f j i) n.
Proof.
  intros Hi.
  unfold degree. 
  f_equal.
...
(* Program : make a list (or just function?) with dimensions 
   equal to that of the relation assigning to each index the
   source and target spider (e.g. 3 to 6), then turn it into
   a bipermutation, proven by defining a WF___ condition for
   this type of list  *)
Fixpoint 
...
(* Import PermutationDefinitions PermutationAutomation  *)
